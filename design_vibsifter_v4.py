...
def calculate_natural_frequency(k_total, m_total):
    if m_total <= 0 or k_total <= 0: return 0.0
    omega_n = np.sqrt(k_total / m_total); f_n = omega_n / (2 * np.pi); return f_n

def calculate_component_amplitude(F_comp, k_total, m_total, f_op, f_n):
    if k_total <= 0 or f_n <= 1e-6 or m_total <=0: return 0.0
    if abs(f_op - f_n) < 1e-6 : return 1.0 # Resonance indicator
    static_def = F_comp / k_total; freq_ratio = f_op / f_n
    denom = abs(1 - freq_ratio**2)
    if denom < 1e-6: return 1.0 # Resonance indicator
    dyn_amp_factor = 1 / denom; amp_m = static_def * dyn_amp_factor; return max(0.0, amp_m)

def calculate_forces(top_moment, bot_moment, lead_angle_deg, freq_hz):
    if freq_hz <=0: return 0.0, 0.0, 0.0
    omega_sq = (2 * np.pi * freq_hz)**2
    F_top = top_moment * omega_sq; F_bottom = bot_moment * omega_sq
    delta_rad = np.radians(lead_angle_deg)
    F_total_sq = F_top**2 + F_bottom**2 + 2*F_top*F_bottom*np.cos(delta_rad)
    F_total = np.sqrt(max(0, F_total_sq)); return F_top, F_bottom, F_total
...
def calculate_steady_state_blocking_f(toss_indicator_K, f_inf_min, f_inf_max, K_sensitivity, K_midpoint):
    ...
    if toss_indicator_K < 0: toss_indicator_K = 0
    exponent_term = -K_sensitivity * (toss_indicator_K - K_midpoint)
    if exponent_term > 700: sigmoid_denominator = 1.0
    elif exponent_term < -700: sigmoid_denominator = float('inf')
    else: sigmoid_denominator = 1.0 + math.exp(exponent_term)
    if sigmoid_denominator == float('inf'): f_inf = f_inf_max
    else: f_inf = f_inf_min + (f_inf_max - f_inf_min) / sigmoid_denominator
...
def simulate_particle_path(diameter, lead_angle_deg, amplitude_eff_mm, step_scale_factor, max_steps=50000):
    ...
    r_current = 0.01 * radius; theta_current = 0.0
    x_path = [r_current * np.cos(theta_current)]; y_path = [r_current * np.sin(theta_current)]
    steps = 0; path_warning = None
    ...
    while r_current < radius and steps < max_steps:
        ...
        if r_current < 1e-6: delta_theta = 0
        else: delta_theta = delta_arc / r_current
    ...
    if r_current < radius and steps == max_steps:
         path_warning = f"Max steps ({max_steps}) reached. Path incomplete (high lead angle?)."
    elif steps < max_steps and len(x_path)>1: # Normalize last step
        try: # Add try-except for potential math errors in normalization
            last_x, last_y = x_path[-1], y_path[-1]; prev_x, prev_y = x_path[-2], y_path[-2]
            dx, dy = last_x - prev_x, last_y - prev_y; dr_sq = dx*dx + dy*dy
            D_intersect = prev_x*dy - prev_y*dx
            discriminant = (radius**2)*dr_sq - D_intersect**2
            if discriminant >= 0 and dr_sq > 1e-9:
                sgn_dy = 1 if dy >= 0 else -1; sqrt_discriminant = np.sqrt(discriminant)
                ...
                dist1_sq = (x1-last_x)**2 + (y1-last_y)**2; dist2_sq = (x2-last_x)**2 + (y2-last_y)**2
                if dist1_sq < dist2_sq: x_path[-1], y_path[-1] = x1, y1
                else: x_path[-1], y_path[-1] = x2, y2
            else: # Fallback if normalization fails
                 x_path[-1] = radius * np.cos(theta_current); y_path[-1] = radius * np.sin(theta_current)
...
blocking_k_midpoint = st.sidebar.number_input(
    "Blocking K Midpoint", min_value=0.5, max_value=10.0, value=BLOCKING_K_MIDPOINT, step=0.1,
...
motor_max_exciting_force_n = st.sidebar.number_input(
    "Motor Max Exciting Force (F_max) (N)", min_value=500.0, max_value=20000.0, value=F_max, step=100.0,
...
top_mass_moment_kgm = st.sidebar.number_input(
    "Top Mass Moment (m*r) (kg·m)", min_value=0.0, max_value=1.50, value=top_ecc_m, step=0.05, format="%.3f",
    help="Influences Horizontal Amplitude."
)
bottom_mass_moment_kgm = st.sidebar.number_input(
    "Bottom Mass Moment (m*r) (kg·m)", min_value=0.0, max_value=1.50, value=bottom_ecc_m, step=0.05, format="%.3f",
    help="Influences Vertical Amplitude."
)
lead_angle_physical_deg = st.sidebar.slider(
    "Physical Lead Angle (δ) (degrees)", min_value=0.0, max_value=90.0, value=lead_angle, step=5.0,
...
stiffness_per_spring_N_m = st.sidebar.number_input(
    "Stiffness per Spring (N/m)", min_value=1000.0, max_value=5000.0, value=stiffness_p, step=1000.0
)
number_of_springs = st.sidebar.number_input( # Corrected typo previously
    "Number of Support Springs", min_value=3, max_value=12, value=6, step=1,
...
frequency_hz = st.sidebar.slider(
    "Operating Frequency (Hz)", min_value=10.0, max_value=90.0, value=freq_current, step=1.0
)
# --- Calculations ---
# Unit conversions
suggested_aperture_mm = recommend_mesh_size(min_granule_mm_for_suggestion) # Use the suggestion input
...
# Forces
F_top_theo_n, F_bottom_theo_n, F_total_theo_n = calculate_forces(top_mass_moment_kgm, bottom_mass_moment_kgm, lead_angle_physical_deg, frequency_hz)
# Force Limiting Check
is_force_limited = False; F_top_actual_n = F_top_theo_n; F_bottom_actual_n = F_bottom_theo_n; F_total_actual_n = F_total_theo_n
if F_total_theo_n > motor_max_exciting_force_n and F_total_theo_n > 1e-6:
    is_force_limited = True; scaling_factor = motor_max_exciting_force_n / F_total_theo_n
    F_top_actual_n = F_top_theo_n * scaling_factor; F_bottom_actual_n = F_bottom_theo_n * scaling_factor
    F_total_actual_n = motor_max_exciting_force_n
F_total_display_kn = F_total_actual_n / 1000.0; motor_max_force_kn = motor_max_exciting_force_n / 1000.0

# Amplitudes
A_h_m = calculate_component_amplitude(F_top_actual_n, k_total_N_m, total_vibrating_mass_kg, frequency_hz, f_n_hz)
A_v_m = calculate_component_amplitude(F_bottom_actual_n, k_total_N_m, total_vibrating_mass_kg, frequency_hz, f_n_hz)

is_resonant_h=A_h_m>=0.999; is_resonant_v=A_v_m>=0.999; is_resonant=is_resonant_h or is_resonant_v
if is_resonant_h: A_h_mm = float('inf')
else: A_h_mm = A_h_m * 1000.0
if is_resonant_v: A_v_mm = float('inf')
else: A_v_mm = A_v_m * 1000.0
...
with col_dyn1: st.metric(label="Total Vib. Mass ($M_{total}$)", value=f"{total_vibrating_mass_kg:.1f} kg"); st.caption(f"Machine ({machine_vibrating_mass_kg}kg) + Material ({material_mass_kg:.1f}kg)")
with col_dyn2: st.metric(label="Natural Freq ($f_n$)", value=f"{f_n_hz:.2f} Hz"); st.caption(r"$f_n \approx \frac{1}{2\pi} \sqrt{k/M}$")
with col_dyn3: st.metric(label="Freq Ratio ($f/f_n$)", value=f"{frequency_ratio:.2f}"); #... resonance warning/info
col_vib1, col_vib2, col_vib3 = st.columns(3)
with col_vib1: st.metric(label="Est. $A_h$ / $A_v$", value=f"{A_h_mm:.2f}/{A_v_mm:.2f} mm" if not is_resonant else "Resonant!"); st.caption(r"$A \approx \frac{F_{act}/k}{|1-(f/f_n)^2|}$")
with col_vib2: st.metric(label="Toss Indicator ($K$)", value=f"{toss_indicator_K:.2f}" if not is_resonant_v else "N/A"); st.caption(r"$K \approx 4\pi^2 f^2 A_v/g$")
with col_vib3: st.metric(label="$F_{actual}$", value=f"{F_total_display_kn:.2f} kN"); force_percentage = (F_total_actual_n / motor_max_exciting_force_n) * 100 if motor_max_exciting_force_n > 0 else 0; st.caption(f"$F_{{act}}$ / $F_{{max}}$ ({motor_max_force_kn:.1f} kN), ~{force_percentage:.0f}%")
if is_force_limited: st.warning("Motor force limited!")
elif F_total_actual_n <= 1e-6: st.info("Enter weights/frequency.")
else: st.success(f"Force Feasible")
...
if plt: # Only show if Matplotlib was imported successfully
    with st.expander(":blue[Visualize Conceptual Flow Pattern]", expanded=False):
        ...
        vis_amp_eff = A_eff_mm_calc if not is_resonant else 1.0 # Use a small default if resonant
...
with st.expander(":blue[Show Blocking Model Formulas]"):
    st.markdown("**Steady-State Blocking Model (`f_∞ = fn(K)`):**"); st.latex(r"K \approx \frac{4\pi^2 f^2 A_v}{g}"); st.caption(f"Toss Indicator K calculated."); st.latex(r"f_{\infty} = f_{min} + \frac{f_{max}-f_{min}}{1+e^{-K_{sens}(K-K_{mid})}}"); st.caption(f"Calculated $f_{{∞}}$ using $K={toss_indicator_K:.2f}$ and tuning params.")
